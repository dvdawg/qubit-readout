...
# sweep parameters
kappa_vals = np.linspace(0.005, 0.05, 20)  # resonator linewidths κ (GHz)
nbar_vals  = np.linspace(0, 180, 91)       # photon number grid
drive_amps = 2.0 * g * np.sqrt(nbar_vals)

# initialize result matrix
transition_prob_matrix = np.zeros((len(kappa_vals), len(nbar_vals)))

# fixed detuning (choose a representative value)
Delta      = 1.0                   # fixed transmon–resonator detuning Δ (GHz)
omega_q    = omega_r + Delta
EJ         = (omega_q + EC)**2 / (8 * EC)

# build transmon Hamiltonians once
num_states   = 12
...
for i, kappa in enumerate(kappa_vals):
    ...
    for j in range(len(nbar_vals)):
        ...
        # transition probability out of |g> = 1 - overlap
