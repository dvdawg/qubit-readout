...
bessel_j0_zeros = np.array([2.4048255577, 5.5200781103, 8.6537279129])
for idx, jz in enumerate(bessel_j0_zeros):
    nbar_line = (jz * Delta_vals / (2 * g)) ** 2
    mask = nbar_line <= nbar_vals[-1]
    plt.plot(Delta_vals[mask], nbar_line[mask], '--', linewidth=1, alpha=0.7, label=f'J0 zero {idx+1}')
nbar_stark = (Delta_vals ** 2) / (g ** 2)
mask = nbar_stark <= nbar_vals[-1]
plt.plot(Delta_vals[mask], nbar_stark[mask], 'c-.', linewidth=1.2, alpha=0.8, label='Δ+χ n̄=0')
